#!/usr/bin/env python3
"""
Detektor kvarova s jednim LightGBM modelom.

Iste multi-view znacajke kao voltage klasifikator:
...
# Datoteke kvarova
FAULT_FILES = OrderedDict([
    ('fan_imbalance', os.path.join(DATA_DIR, 'imbalance_fan_data.json')),
    ('movement',      os.path.join(DATA_DIR, 'movement_data.json')),
    ('voltage_spike', os.path.join(DATA_DIR, 'voltage_spikes_data.json')),
])

NORMAL_FILES = OrderedDict([
...
CLASSES = ['normal', 'fan_imbalance', 'movement']
CLASS_TO_IDX = {c: i for i, c in enumerate(CLASSES)}

# -- Znacajke (iste 48 kao voltage klasifikator) --------------------------
RAW_FEATURES = [
...
def load_normal_data():
    """Ucitava sve normalne (ispravne) podatke za sve napone."""
    podaci = []
    for napon, lista_datoteka in NORMAL_FILES.items():
        for naziv_datoteke in lista_datoteka:
            ...
            if os.path.exists(putanja):
                ...
                podaci.append(niz)
                print(f"Normalni {napon}: {naziv_datoteke} -> {len(niz)} uzoraka")
            else:
                print(f"ERROR: {naziv_datoteke} nije pronadena")
    return np.concatenate(podaci, axis=0) if podaci else np.array([])


def load_fault_data():
    """Ucitava podatke kvarova: fan_imbalance, movement. voltage_spike se analizira posebno."""
    podaci_kvarova = {}
    for klasa_kvara, putanja in FAULT_FILES.items():
        if os.path.exists(putanja):
            niz = load_ndjson(putanja, RAW_FEATURES, RATIO_DEFS)
            podaci_kvarova[klasa_kvara] = niz
            print(f"Kvar {klasa_kvara}: {len(niz)} uzoraka")
        else:
            print(f"ERROR: {klasa_kvara} datoteka nije pronadena: {putanja}")
    return podaci_kvarova


# -- Windowing & transforms ------------------------------------------------
def make_windows(data, window_size, stride=1):
    uzorci, broj_znacajki = data.shape
    if uzorci < window_size:
        print(f"ERROR: podaci imaju {uzorci} uzoraka, window_size={window_size}")
        return np.array([])
...
def analyze_voltage_spikes(podaci_skokova):
    ...
    indeks_a_rms = 11
    vrijednosti_a_rms = podaci_skokova[:, indeks_a_rms]
    srednja_a_rms = np.mean(vrijednosti_a_rms)
    std_a_rms = np.std(vrijednosti_a_rms)

    # peak_peak znacajke su na indeksima 12, 13, 14 (x, y, z)
    pp_x = podaci_skokova[:, 12]
    pp_y = podaci_skokova[:, 13]
    pp_z = podaci_skokova[:, 14]
    ...
    pragovi = {
        ...
        'pp_min': float(pp_min),
        'pp_max': float(pp_max),
        'pp_jump_mean': float(srednji_skok_pp),
        'pp_jump_max': float(maksimalni_skok_pp),
        'note': f'Calibrated from {len(podaci_skokova)} voltage spike samples',
    }
...
def main():
    ...
    # Normalna klasa
    broj_trening_uzoraka = int(len(normalni_podaci) * TRAIN_RATIO)
    trening_podaci = normalni_podaci[:broj_trening_uzoraka]
    test_podaci = normalni_podaci[broj_trening_uzoraka:]
    ...
    lista_trening_oznaka.append(np.full(len(trening_prozori), CLASS_TO_IDX['normal'], dtype=np.int64))
    lista_test_prozora.append(test_prozori)
    lista_test_oznaka.append(np.full(len(test_prozori), CLASS_TO_IDX['normal'], dtype=np.int64))

    print(f"normal: trening={len(trening_prozori)}, test={len(test_prozori)}")

    # Klase kvarova (fan_imbalance, movement)
    for klasa_kvara in ['fan_imbalance', 'movement']:
        if klasa_kvara in podaci_kvarova:
            podaci = podaci_kvarova[klasa_kvara]
            indeks_klase = CLASS_TO_IDX[klasa_kvara]
            broj_trening_uzoraka_kvara = int(len(podaci) * TRAIN_RATIO)

            trening_podaci_kvara = podaci[:broj_trening_uzoraka_kvara]
            test_podaci_kvara = podaci[broj_trening_uzoraka_kvara:]

            trening_prozori_kvara = make_windows(trening_podaci_kvara, WINDOW_SIZE, stride=1)
            test_prozori_kvara = make_windows(test_podaci_kvara, WINDOW_SIZE, stride=1)

            lista_trening_prozora.append(trening_prozori_kvara)
            lista_trening_oznaka.append(np.full(len(trening_prozori_kvara), indeks_klase, dtype=np.int64))
            lista_test_prozora.append(test_prozori_kvara)
            lista_test_oznaka.append(np.full(len(test_prozori_kvara), indeks_klase, dtype=np.int64))

            print(f"{klasa_kvara}: trening={len(trening_prozori_kvara)}, test={len(test_prozori_kvara)}")
        else:
            print(f"{klasa_kvara}: NIJE PRONAdENO")
    ...
    print(f"Spremljeno {putanja_modela}")

    nazivi_pogleda = ['raw', 'minmax', 'log', 'diff']

    konfiguracija_kvara = {
        'version': 'fault_detector',
        'ensemble_type': 'multiview_fault_detector',
        'classes': CLASSES,
    ...
    with open(putanja_konfiguracije, 'w') as datoteka:
        json.dump(konfiguracija_kvara, datoteka, indent=2)
